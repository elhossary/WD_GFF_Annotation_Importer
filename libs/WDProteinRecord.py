...
class WDProteinRecord(WDBioGffRecord):
    ...
    def get_WD_desc(self, sub_types, pseudo_flag):
        ...
        child_desc = ""
        if pseudo_flag:
            parent_desc = 'Microbial gene (pseudogene) encodes a protein found in ' + self.Organism_Name \
                          + super().get_desc_adds()
            child_desc = 'Microbial protein encoded by a pseudogene found in ' + self.Organism_Name \
                         + super().get_desc_adds()
        else:
            parent_desc = 'Microbial gene encodes a protein found in ' + self.Organism_Name + super().get_desc_adds()
            child_desc = 'Microbial protein found in ' + self.Organism_Name + super().get_desc_adds()
    ...
    def make_WD_definitions(self, sub_types, pseudo_flag):
        ...
        for sub_type in sub_types:
            if sub_type == 'protein':
                r_list_parent.append({'property': 'P279', 'target': 'Q20747295'}) # subclass of protein coding gene
            elif sub_type == '':
                r_list_parent.append({'property': 'P279', 'target': ''})# for further use
            else:
...
# ---------------------------------------------------------------------------------------------------------------------
        ...
        r_list_child.append({'property': 'P31', 'target': 'Q8054'})  # instance of protein
        r_list_child.append({'property': 'P279', 'target': 'Q8054'})  # subclass of protein
