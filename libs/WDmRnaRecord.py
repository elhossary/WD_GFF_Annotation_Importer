...
class WDmRnaRecord(WDBioGffRecord):
    ...
    def get_WD_desc(self, sub_types, pseudo_flag):
        ...
        child_desc = ""
        if pseudo_flag:
            parent_desc = 'Microbial gene (pseudogene) encodes a mRNA found in ' + self.Organism_Name \
                          + super().get_desc_adds()
            child_desc = 'Microbial mRNA encoded by a pseudogene found in ' + self.Organism_Name \
                         + super().get_desc_adds()
        else:
            parent_desc = 'Microbial gene encodes a mRNA found in ' + self.Organism_Name + super().get_desc_adds()
            child_desc = 'Microbial mRNA found in ' + self.Organism_Name + super().get_desc_adds()
    ...
    # -----------------------------------------------------------------------------------------------------------------
        ...
        for sub_type in sub_types:
            if sub_type == 'mRNA':
                r_list_child.append({'property': 'P279', 'target': 'Q188928'})  # subclass of mRNA
            # elif sub_type == '':
            #  r_list_child.append({'property': 'P279', 'target': ''})  # for further use
            else:
